...
struct aesd_buffer_entry *aesd_circular_buffer_find_entry_offset_for_fpos(struct aesd_circular_buffer *buffer, size_t char_offset,size_t *entry_offset_byte_rtn) {
  int search = buffer->out_offs;
  for (int i = 0; i < AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED; i++) {
    if ((buffer->entry[search].size) >= (char_offset + 1)) {
      *entry_offset_byte_rtn = char_offset;
      return &(buffer->entry[search]);
    } else {
      char_offset -= buffer->entry[search].size;
    }
    search = (search + 1) % AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED;
  }
  return NULL;
}
